...
from algorithm.evolutionary_computing.symbolic_regression.operator_definitions import OPERATOR_NAMES
...
class ComponentGenerator:
    ...
    def add_operator(self, operator_to_add, operator_weight=None):
        """Add an operators number to the set of possible operators

        Parameters
        ----------
        operator_to_add : int, str
            operators integer code (e.g. 2, 3) defined in Agraph operators maps
            or an operators string description (e.g. "+", "addition")
        operator_weight : number
                          relative weight of operators probability
        """
    ...
    def _get_operator_number_from_string(operator_string):
        ...
        raise ValueError(f"Could not find operators {operator_string}. ")
    ...
    def random_operator_command(self, stack_location):
        """Get a random operators (non-terminal) command
    ...
    def random_operator(self):
        """Get a random operators

         Get a random operators from the list of possible operators.
    ...
    def random_operator_parameter(stack_location):
        """Get random operators parameter
        ...
        -------
        int
            parameter to be used in an operators command


        Notes
        -----
        The returned random operators parameter is guaranteed to be less than
        stack_location.
        """
        return np.random.randint(stack_location)

    def random_terminal_command(self, _=None):
        """Get a random terminal (non-operators) command
